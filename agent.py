...
_TOOLS = [
    ...
    {"name": "search_restaurants", "description": "Search Google Places (primary), Yelp, and TripAdvisor for restaurants near a location. Only returns results with a 4.0+ rating and 100+ reviews, labeled by source (a source is skipped/noted if unconfigured or erroring, e.g. Yelp requiring a paid plan). Pass a specific town/suburb/neighborhood as location, not a wide region.", "input_schema": {"type": "object", "properties": {"location": {"type": "string"}, "term": {"type": "string"}, "price": {"type": "string"}}, "required": ["location"]}},
