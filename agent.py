...
_TOOLS = [
    ...
    {"name": "search_restaurants", "description": "Search Google Places (primary, 4.5+ rating / 100+ reviews), Yelp (4.0+ rating / 100+ reviews), and TripAdvisor (shown unfiltered, reference only) for restaurants near a location, labeled by source (a source is skipped/noted if unconfigured or erroring, e.g. Yelp requiring a paid plan). Pass a specific town/suburb/neighborhood as location, not a wide region.", "input_schema": {"type": "object", "properties": {"location": {"type": "string"}, "term": {"type": "string"}, "price": {"type": "string"}}, "required": ["location"]}},
    {"name": "get_restaurant_details", "description": "Look up a specific restaurant by name + location across Google, Yelp, and TripAdvisor for rating, review count, and phone number, labeled by source.", "input_schema": {"type": "object", "properties": {"name": {"type": "string"}, "location": {"type": "string"}}, "required": ["name", "location"]}},
