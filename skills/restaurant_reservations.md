...
## Inputs
- `search_restaurants`, `get_restaurant_details` — headless-browser lookups
  (no API keys) against Google Maps (primary), Yelp, and TripAdvisor for
  rating/review-count verification; results come back labeled by source.
  This is scraping, not an API — a source can occasionally fail to parse or
  get blocked, in which case it shows up as a labeled note rather than a
  hard error. It does not return phone numbers.
...
2. Use `search_restaurants` with a specific town/suburb inside the correct
   boundary (not the whole region at once) to find candidates. It reads
   Google Maps (primary, 4.5+ rating / 100+ reviews) and Yelp (4.0+ rating /
   100+ reviews) as hard filters — never propose a restaurant that doesn't
   meet its source's bar, or that you haven't verified with
   ...
   source. If a source failed to parse or got blocked, say so briefly rather
   than pretending it wasn't tried.
...
4. **Booking order** — once a restaurant is picked:
   a. Ask the owner to confirm whether the restaurant takes Resy or
      OpenTable bookings — you don't have direct Resy/OpenTable account
      access, so you can't silently browse either platform, and the lookup
      tools don't surface reservation links.
   b. If neither applies (or the owner isn't sure), fall back to calling the
      restaurant directly via `make_call`, per the phone-calls skill: get
      the phone number from the owner (don't guess it), then state who
      you'll call, the number, and the objective ("reserve a table for
      [party size] at [time] on [date] under the name [owner]"), then wait
      for explicit approval before calling.
